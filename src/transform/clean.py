...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        DataFrame pronto para carga no BigQuery
    """
    logger.info("Iniciando transformação...")
    df = df.copy()

    # 1. Cast de tipos
    df["taxa_mortalidade_infantil"] = pd.to_numeric(
        df["taxa_mortalidade_infantil"], errors="coerce"
    )
    ...
    # 2. Remover nulos
    linhas_antes = len(df)
    df = df.dropna(subset=["taxa_mortalidade_infantil"])
    df = df[df["taxa_mortalidade_infantil"] > 0]
    logger.info(f"Removidas {linhas_antes - len(df)} linhas nulas/inválidas")

    # 3. Filtrar apenas UFs válidas
...
def _validar(df: pd.DataFrame) -> None:
    """Validações básicas de qualidade — levanta exceção se falhar."""
    assert len(df) > 0, "DataFrame vazio após transformação"
    assert df["taxa_mortalidade_infantil"].between(0, 200).all(), \
        "Taxa de mortalidade fora do intervalo esperado (0–200)"
